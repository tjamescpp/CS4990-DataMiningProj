...
def main(fname, k=5, width=256, height=256, location=TOP, redraw=True):
    ...
    with Image.open(fname) as im:
        ...
        centers, labels = clustering.kmeans(data, k, columns=[0, 1, 2], n=20)
        if not redraw:
            # Show cluster centers as color palette
            for i, c in enumerate(centers):
                pal = Image.new("RGB", (PALETTE_SIZE, PALETTE_SIZE), tuple(
                    map(lambda x: int(round(x)), c)))
                if location == TOP:
...
if __name__ == "__main__":
    ...
    main(args.filename, k=args.clusters, width=args.width,
         height=args.height, location=args.left, redraw=args.redraw)
