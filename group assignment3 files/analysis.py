...
for k in k_values:
    medoids = kmedoids(column, k, distance=manhattan_distance, n=100, eps=1e-3)
    clusters_kmedoids = [
        min(range(k), key=lambda idx: manhattan_distance(point, medoids[idx]))
        for point in column
    ]
    score = silhouette_score(cancer_data[['PC1', 'PC2']], clusters_kmedoids)
