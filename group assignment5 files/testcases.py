...
CLASSIFICATION_TESTS = ["Predict class from two categories (1+2)", "Predict class from two categories (3+4)", "Predict class from all four categories", "Predict class from three categories (2-4)",
...
MODELS = {"Decision Tree": classification.DecisionTree,
...
def main(auto=False, nb=False, steps=[]):
    random.seed(1337)
    df = pandas.read_csv("group assignment5 files/testdata.csv")
    model = "Decision Tree"
    if nb:
        model = "Naive Bayes"
    training = []
